...
export function ConnectionMarquee({ items }: { items: string[] }) {
  ...
  return <section className="marquee-band page-wrap" aria-labelledby="open-threads-heading">
    <div className="marquee-heading">
      <div><span className="meta-line" id="open-threads-heading">OPEN THREADS</span><p>People, places, and practices in motion.</p></div>
      <button className="marquee-control" type="button" onClick={() => setPaused((value) => !value)} aria-pressed={paused}>{paused ? "Play" : "Pause"}</button>
    </div>
    <p className="sr-only">Open threads: {items.join(", ")}</p>
