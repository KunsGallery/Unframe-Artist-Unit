...
export default function DashboardProfilePage() {
  ...
  return <main className="dashboard-page"><DemoNotice /><div className="dashboard-wrap"><DashboardSidebar active="profile" /><section className="dashboard-main profile-main">
    ...
    <div className="profile-layout"><form className="profile-form" onSubmit={handleSubmit}><div className="profile-form-intro"><MetaLine>{tx(locale, "YOUR IDENTITY", "당신의 정체성")}</MetaLine><p>{tx(locale, "Choose the role that best describes how you enter the unit. You can change it as your practice moves.", "유닛에 들어오는 방식을 가장 잘 설명하는 역할을 선택하세요. 실천의 방향에 따라 언제든 바꿀 수 있습니다.")}</p></div>
      ...
      <div className="profile-media-fields"><MetaLine>{tx(locale, "MEDIA / R2", "미디어 / R2")}</MetaLine><p>{tx(locale, "Upload the images that introduce your practice. They are stored in Cloudflare R2 and only the public URL is connected to your profile.", "작업을 소개하는 이미지를 업로드하세요. 이미지는 Cloudflare R2에 저장되고 공개 URL만 프로필에 연결됩니다.")}</p><div className="profile-media-grid"><R2ImageUploader value={form.profileImageUrl} assetType="profile" entityId={user.uid} label={tx(locale, "Profile image", "프로필 이미지")} description={tx(locale, "A small image for your public identity.", "공개 프로필에 사용할 이미지입니다.")} onChange={(url) => setField("profileImageUrl", url)} /><R2ImageUploader value={form.siteCoverImageUrl} assetType="cover" entityId={user.uid} label={tx(locale, "Site cover", "페이지 커버")} description={tx(locale, "The first image in your public room.", "공개 페이지 첫 화면에 사용할 이미지입니다.")} onChange={(url) => setField("siteCoverImageUrl", url)} /></div></div>
