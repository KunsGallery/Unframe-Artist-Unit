...
export default function AdminClient() {
  ...
  const pendingRoleRequests = useMemo(() => (collections.users ?? []).filter((item) => item.accessStatus === "pending" && item.accountType !== "artist"), [collections.users]);
  ...
  async function reviewRole(uid: string, next: "approved" | "rejected") {
    if (!db) return;
    setActionError(null);
    try {
      await updateDoc(doc(db, "users", uid), { accessStatus: next, approvedAt: next === "approved" ? serverTimestamp() : null });
      ...
      setNotice(tx(locale, next === "approved" ? "Role access approved." : "Role request put on hold.", next === "approved" ? "역할 승인이 완료되었습니다." : "역할 신청을 보류했습니다."));
    } catch (reviewError) {
      setActionError(reviewError instanceof Error ? reviewError.message : "Role access could not be updated.");
    }
  }
