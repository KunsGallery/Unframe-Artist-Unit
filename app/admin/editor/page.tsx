...
export default function AdminEditorPage() {
  ...
  return <main className="admin-editor-page"><DemoNotice /><div className="admin-editor-shell">
    ...
    <div className="admin-editor-layout"><aside className="admin-editor-controls"><div className="admin-editor-control-head"><div><MetaLine>{tx(locale, "LIVE CONTROLS", "실시간 컨트롤")}</MetaLine><h2>{tx(locale, "Tune the rhythm.", "리듬을 조절하세요.")}</h2></div><SlidersHorizontal size={18} /></div><p className="admin-editor-copy">{tx(locale, "Move a control and the public preview updates immediately. Publish when the page feels right.", "컨트롤을 움직이면 공개 미리보기가 즉시 바뀝니다. 페이지가 마음에 들면 발행하세요.")}</p>
      ...
      <label className="editor-range"><span>{tx(locale, "Hero height", "첫 화면 높이")}<output>{draft.heroHeight}px</output></span><input type="range" min="520" max="980" step="10" value={draft.heroHeight} onChange={(event) => updateSetting("heroHeight", Number(event.target.value))} /></label>
      <label className="editor-range"><span>{tx(locale, "Hero gap", "첫 화면 간격")}<output>{draft.heroGap}px</output></span><input type="range" min="30" max="220" step="5" value={draft.heroGap} onChange={(event) => updateSetting("heroGap", Number(event.target.value))} /></label>
      <label className="editor-range"><span>{tx(locale, "Hero title scale", "첫 화면 제목 크기")}<output>{Math.round(draft.heroCopyScale * 100)}%</output></span><input type="range" min="0.72" max="1.4" step="0.01" value={draft.heroCopyScale} onChange={(event) => updateSetting("heroCopyScale", Number(event.target.value))} /></label>
      <label className="editor-range"><span>{tx(locale, "Body line height", "본문 줄 간격")}<output>{draft.bodyLeading.toFixed(2)}</output></span><input type="range" min="1.15" max="1.9" step="0.01" value={draft.bodyLeading} onChange={(event) => updateSetting("bodyLeading", Number(event.target.value))} /></label>
      <label className="editor-range"><span>{tx(locale, "Grid gap", "카드 간격")}<output>{draft.gridGap}px</output></span><input type="range" min="0" max="55" step="1" value={draft.gridGap} onChange={(event) => updateSetting("gridGap", Number(event.target.value))} /></label>
      <label className="editor-range"><span>{tx(locale, "Corner radius", "모서리 둥글기")}<output>{draft.radius}px</output></span><input type="range" min="0" max="30" step="1" value={draft.radius} onChange={(event) => updateSetting("radius", Number(event.target.value))} /></label>
      <label className="editor-range"><span>{tx(locale, "Image saturation", "이미지 채도")}<output>{Math.round(draft.imageSaturation * 100)}%</output></span><input type="range" min="0.35" max="1.5" step="0.01" value={draft.imageSaturation} onChange={(event) => updateSetting("imageSaturation", Number(event.target.value))} /></label>
