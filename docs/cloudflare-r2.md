...
이미지 업로드의 로그인 토큰 검증은 Firebase 공개 서명키를 사용합니다. 조직 정책으로 서비스 계정 키 생성을 제한한 Firebase/Google Cloud 프로젝트에서도 업로드 기능을 사용할 수 있으며, 이 업로드 기능은 `FIREBASE_ADMIN_CLIENT_EMAIL`과 `FIREBASE_ADMIN_PRIVATE_KEY`를 읽지 않습니다. 기존에 등록한 값이 잘못되어도 영향을 받지 않습니다.
